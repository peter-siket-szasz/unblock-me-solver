...
exports.getMoves = exports.getVerticalMoves = exports.getHorizontalMoves = void 0;
/**
 ...
 */
function getHorizontalMoves(block, blocks, maxWidth = 6, bidirectional = false) {
    const { id, y, width, height } = block;
    let { x } = block;
    const moves = [];
    if (!bidirectional && width < height)
        return [];
    ...
    while (x > 0) {
        x--;
        !otherBlocks.some((otherBlock) => otherBlock.x + otherBlock.width - 1 === x &&
    ...
    while (x + width < maxWidth) {
        x++;
        !otherBlocks.some((otherBlock) => otherBlock.x === x + width - 1 && !(otherBlock.y >= y + height || otherBlock.y + otherBlock.height <= y))
            ? moves.push({ id, amount: x - block.x, dir: 'x' })
            : (x = maxWidth);
    }
    return moves;
}
exports.getHorizontalMoves = getHorizontalMoves;
/**
 * @param block The block to move
 * @param blocks The blocks on the board (can include the block to move)
 ...
 * @returns The possible moves for the block as an array of Move objects
 */
function getVerticalMoves(block, blocks, maxHeight = 6, bidirectional = false) {
    const { id, x, width, height } = block;
    let { y } = block;
    const moves = [];
    if (!bidirectional && height < width)
        return [];
    // Filter out the moving block itself
    const otherBlocks = blocks.filter((b) => b.id !== block.id);
    // Moves upwards
    while (y > 0) {
        y--;
        !otherBlocks.some((otherBlock) => otherBlock.y + otherBlock.height - 1 === y &&
    ...
    while (y + height < maxHeight) {
        y++;
        !otherBlocks.some((otherBlock) => otherBlock.y === y + height - 1 && !(otherBlock.x >= x + width || otherBlock.x + otherBlock.width <= x))
            ? moves.push({ id, amount: y - block.y, dir: 'y' })
            : (y = maxHeight);
    }
    return moves;
}
exports.getVerticalMoves = getVerticalMoves;
/**
 * @param block The block to move
 * @param blocks The blocks on the board (can include the block to move)
 ...
 * @returns The possible moves for the block as an array of Move objects
 */
function getMoves(block, blocks, maxWidth = 6, maxHeight = 6, bidirectional = false) {
